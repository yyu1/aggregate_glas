...
out_file = '/Volumes/wd_3t/maxent/v5/samples/maxent_agb_v5_sam_20pctslope_2shotmin_250modgrid_v2.csv'
...
foreach value, glas_hash_table, key do begin
	n_shots = n_elements(value)
	if (n_shots gt 1) then begin
		outline = ''
		cur_xpix = ulong64(key/10000000ULL)
		cur_ypix = key - cur_xpix*10000000ULL
		cur_modcoord = sinpix_to_250m_sinxy(cur_xpix, cur_ypix)
		coord_format = '(f12.2)'
		hlorey_format = '(f6.2)'

		;hlorey_total = total(glas_hash_table[hash_keys[i]])
		hlorey_avg = mean(value)
